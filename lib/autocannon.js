...
const body = JSON.stringify({
  query:
    `query {
  findNote1s(filter: { not: {title:{ contains: "123" } }}) {
    items {
      id
      title
      description
...
module.exports.fire = async (opts, handler, save, startupTime) => {
  var result = await run(opts);
  result.startupTime = startupTime;
